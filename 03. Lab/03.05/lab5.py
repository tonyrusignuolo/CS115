...
def fastED(first, second):
    ...
    def fed_helper(first, second, memo):
        ...
        if first == '':
            ...
            result = fed_helper(first[1:],second[1:], memo)
        else:
            substitution = 1 + fed_helper(first[1:], second[1:],memo) 
            deletion = 1 + fed_helper(first[1:], second,memo) 
            insertion = 1 + fed_helper(first, second[1:],memo) 
            result = min(substitution, deletion, insertion)
        memo[(first, second)] = result
        return result
    return fed_helper(first,second,{})
