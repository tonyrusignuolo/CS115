...
class Star(object):
    ...
    def distance_to(self, other):
        '''Computes the distance to the other Star passed as an 
        argument to the method.
        Recall the distance formula (not written in valid Python 
        syntax): d = sqrt( (x2 - x1)^2 + (y2 - y1)^2 )
        '''
...
class Sky(object):
    ...
    def westernmost_star(self):
        ...
        value).'''
        westermost = self.__star[0]
        for i in range(1,len(self.__stars)):
            if self.__stars[i].x < westermost.x:
